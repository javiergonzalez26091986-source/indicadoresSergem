...
def cambiar_pagina(nombre_pagina):
    # Validación: Si intenta ir a un tablero sin haber cargado datos
    if nombre_pagina != 'Inicio' and 'df_datos' not in st.session_state:
...
""", unsafe_allow_html=True)
    ...
    with col_der:
        ...
        # Grid de botones para los 4 menús
        ...
        # Mostrar mensaje si ya hay datos en memoria para no confundir al usuario
        if 'df_datos' in st.session_state:
            st.info("✅ **Archivo ya cargado en memoria.** Puede navegar por los tableros arriba, o subir uno nuevo para reemplazarlo.")
            
        archivo_subido = st.file_uploader("📥 Subir archivo ORIGINAL WIP", type=['xlsx', 'xls', 'csv'])
        
        # PROCESAMIENTO INMEDIATO AL SUBIR EL ARCHIVO
        if archivo_subido is not None:
            try:
                # Lectura del archivo
                if archivo_subido.name.endswith('.csv'):
                    df_temp = pd.read_csv(archivo_subido, sep=';', encoding='utf-8', on_bad_lines='skip')
                else:
                    df_temp = pd.read_excel(archivo_subido)
                    
                df_temp.columns = df_temp.columns.str.strip().str.upper()
                
                # Procesamiento de fechas de una sola vez
                if 'FECHA DE CREACION' in df_temp.columns:
                    df_temp['FECHA DE CREACION'] = pd.to_datetime(df_temp['FECHA DE CREACION'], errors='coerce')
                    df_temp = df_temp.dropna(subset=['FECHA DE CREACION'])
                    ...
                    df_temp['DIA_SEMANA'] = pd.Categorical(df_temp['FECHA DE CREACION'].dt.day_name().map(dias_esp), categories=orden_dias, ordered=True)
                    df_temp['MES'] = df_temp['FECHA DE CREACION'].dt.to_period('M').astype(str)

                # Procesamiento de tiempos de una sola vez
                if 'TIEMPO EJECUCIÓN REAL' in df_temp.columns:
                    df_temp['TIEMPO_MINUTOS'] = df_temp['TIEMPO EJECUCIÓN REAL'].apply(convertir_a_minutos)
                    df_temp['TIEMPO_HORAS'] = df_temp['TIEMPO_MINUTOS'] / 60 
                
                # Guardamos el DataFrame final en memoria (session_state)
                st.session_state['df_datos'] = df_temp
                st.session_state['alerta_archivo'] = False 
                st.success("¡Datos procesados y guardados en memoria exitosamente! Seleccione un tablero arriba.")
